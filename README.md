...
> Строить карту на только на втором лазере нормально не получалось тк он смотрит только в одну сторону, достаточно недалеко => частато обновления маленькая, получается много артефактов.
Позже выяснилось, что если бы мы фиксировали эти данные, то ориентация по такой карте с помощью 2D slam была бы практически невозможна.
